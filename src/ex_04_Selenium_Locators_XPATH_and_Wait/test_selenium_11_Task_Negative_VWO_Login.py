...
// Locators - Find the Web elements with XPATH and Use explicit wait
...
@allure.description("Verify error message is shown with wrong user credentials.")
def test_selenium_VWO_invalid_login():
    ...
    # Explicit wait: continuously checks the web element is visible or not.
    WebDriverWait(driver = driver, timeout = 5).until(EC.visibility_of_element_located((By.XPATH, "//div[@id='js-notification-box']")))
